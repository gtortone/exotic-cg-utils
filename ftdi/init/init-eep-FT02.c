...
int main(void) {
   ...
   if (ftdi_eeprom_initdefaults(ftdi, "FTDI", "FTDI Quad channel", "FT02") < 0) {
      printf("ftdi_eeprom_initdefaults: ERROR");
      ftdi_free(ftdi);
      return -1;
   }

   if (ftdi_set_eeprom_value(ftdi, VENDOR_ID, 0x0403) < 0) {    
      printf("ftdi_set_eeprom_value: (%s)\n", ftdi_get_error_string(ftdi));
      ftdi_free(ftdi);
      return -1;
   }
 
   if (ftdi_set_eeprom_value(ftdi, PRODUCT_ID, 0x6011) < 0) {    
      printf("ftdi_set_eeprom_value: (%s)\n", ftdi_get_error_string(ftdi));
      ftdi_free(ftdi);
      return -1;
   }
